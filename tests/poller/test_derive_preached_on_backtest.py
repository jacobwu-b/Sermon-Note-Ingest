...
the spec's "PBC published_at" research (see the #51 discussion) rather than
silently dropped:

- PBC's archive-backfill records: their ``published_on`` came from an S3 path
...
- **All** of PBC's live records: PBC's ``published_at`` comes from the audio
  file's CDN ``Last-Modified`` header (pbc.py's ``resolve_published_at``), which
  ...
  records (45%) have a ``published_at`` more than a day after ``published_on``,
  ...
  PBC's own ``published_on`` is already reliable (100% Sunday across every live
  ...
  the place to resolve what *should* anchor PBC's ``published_at`` (a separate,
...
# Only Westgate and GracePres derive published_on from the title; every other
...
# GracePres's published_at is a SoundCloud upload instant in UTC, meaningless as
# a calendar day without converting to the church's own timezone first (see the
# module docstring's backtest finding). Every other church's published_at is
# already in a timezone where its own calendar day is the relevant one.
_TZ_BY_CHURCH = {"gracepres": _GRACEPRES_TZ}

# PBC's published_at is excluded entirely — see the module docstring.
    ...
    for path in sorted(_DATA_DIR.glob("*.json")):
        ...
        for guid, record in records.items():
            if guid.startswith("pbc-archive:"):
                continue
            if not record.get("published_at") or not record.get("published_on"):
                continue
...
def test_derive_preached_on_matches_every_historical_live_record():
    ...
    for church, guid, record in _live_records():
        ...
        computed = derive_preached_on(published_at, title_date=title_date, tz=tz)
        stored = date.fromisoformat(record["published_on"])
        if computed != stored:
            mismatches.append(
    ...
    assert not mismatches, (
        f"derive_preached_on disagrees with the stored published_on for "
