"""Deterministic title → filename-stem slug (PRD §7.3).

The rendered artifact is named ``notes/YYYY/YYYY-MM-DD_<slug>.docx``; this module
owns the ``<slug>`` half. The only invariant the pipeline depends on is
determinism — the same title always produces the same stem, so a re-run targets the
same file (PRD §11.3) — plus a stem that is safe on any filesystem: ASCII,
